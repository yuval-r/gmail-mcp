...
| `search_all_accounts` | `query`, `max_results_per_account=10`, `page_tokens?` | One search across **every** account, each result tagged by account. When accounts have more matches, the output ends with a `page_tokens` map (account to token); pass it back to continue just those accounts. |
...
| `read_messages` | `account`, `message_ids` \| `query`, `max_results=25` (at most 100), `max_body_chars?` | Batch-read many messages in one call (vs. N `read_message` calls). Each body is capped like `read_message`; pass `max_body_chars=0` for full bodies. |
